...
export async function createRenderer(host, world) {
  ...
  for(const p of POINTS){const y=terrainHeight(p.x,p.z),m=billboard(keyMap,p.x,p.z,.62,.8,0xffedc0,y+.18);m.material.dispose();m.material=new THREE.MeshBasicMaterial({map:keyMap,color:0xffe6a3,transparent:true,alphaTest:.08,side:THREE.DoubleSide,depthTest:false});m.renderOrder=4;m.castShadow=false;const pool=disk(warmMap,p.x,p.z,1.75,1.75,.38,y+.02);items.push({kind:'key',p,m,pool});}
